...
import { useDashboardStats, useRecentActivity, useUpcomingEvents, useFeeOverview, useTaskOverview, useSyllabusOverview } from '../../hooks/useDashboard';
    ...
    >
        ...
        {isStudent && syllabusOverview && (
          <View style={styles.section}>
            ...
            <Card variant="elevated" style={styles.syllabusCard}>
              {syllabusOverview.totalSubjects > 0 && syllabusOverview.subjectBreakdown.length > 0 ? (
                <>
                  <View style={styles.syllabusOverallContainer}>
                    <View style={styles.syllabusProgressRing}>
                      <ProgressRing
                    ...
                    <View style={styles.syllabusOverallDetails}>
                      <Text style={styles.syllabusOverallLabel}>Overall Progress</Text>
                      <Text style={styles.syllabusOverallSubtext}>
                  ...
                  {syllabusOverview.subjectBreakdown.length > 0 && (
                    <View style={styles.syllabusBreakdown}>
                      {syllabusOverview.subjectBreakdown.slice(0, 3).map((subject) => (
                        <View key={subject.subjectId} style={styles.syllabusSubjectItem}>
                          <View style={styles.syllabusSubjectHeader}>
                            <Text style={styles.syllabusSubjectName} numberOfLines={1}>
                              {subject.subjectName}
                            </Text>
                            <Text style={styles.syllabusSubjectProgress}>
                              {subject.progress}%
                            </Text>
                          </View>
                          <View style={styles.syllabusSubjectProgressBar}>
                            <View 
                              style={[
                                styles.syllabusSubjectProgressFill,
                                { 
                                  width: `${subject.progress}%`,
                                  backgroundColor: subject.progress >= 80 ? colors.success[600] : 
                                                  subject.progress >= 50 ? colors.info[600] : colors.warning[600]
                                }
                              ]} 
                            />
                          </View>
                          <Text style={styles.syllabusSubjectSubtext}>
                      ...
                      {syllabusOverview.subjectBreakdown.length > 3 && (
                        <TouchableOpacity 
                          style={styles.syllabusViewMore}
                          onPress={() => router.push('/syllabus')}
                        >
                          <Text style={styles.syllabusViewMoreText}>
                ...
                <View style={styles.emptyStateContainer}>
                  <View style={styles.emptyStateIcon}>
                    <NotebookText size={40} color={colors.primary[400]} />
                  </View>
                  <Text style={styles.emptyStateTitle}>No syllabus data yet</Text>
                  <Text style={styles.emptyStateText}>
                    Syllabus progress will appear here once your teacher starts tracking course completion.
                  </Text>
                  <TouchableOpacity 
                    style={styles.emptyStateCTA}
                    onPress={() => router.push('/syllabus')}
                  >
                    <Text style={styles.emptyStateCTAText}>View Syllabus</Text>
                  </TouchableOpacity>
                </View>
              )}
            </Card>
...
const styles = StyleSheet.create({
  ...
  syllabusOverallValue: {
    position: 'absolute',
    fontSize: typography.fontSize.lg,
    fontWeight: typography.fontWeight.bold,
    color: colors.text.primary,
  },
  syllabusOverallDetails: {
    flex: 1,
    marginLeft: spacing.lg,
  },
  syllabusOverallLabel: {
    fontSize: typography.fontSize.xl,
    fontWeight: typography.fontWeight.bold,
    color: colors.text.primary,
    marginBottom: 4,
  },
  syllabusOverallSubtext: {
    fontSize: typography.fontSize.sm,
    color: colors.text.secondary,
  },
  syllabusBreakdown: {
  ...
  syllabusSubjectProgressFill: {
    height: '100%',
    borderRadius: borderRadius.full,
  },
  syllabusSubjectSubtext: {
    fontSize: typography.fontSize.xs,
    color: colors.text.secondary,
  },
  syllabusViewMore: {
